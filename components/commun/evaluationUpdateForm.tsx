...
const EvalForm = ({ settings }: any) => {
  ...
  const initialFormValues: any = {
    title: settings?.evaluation?.title || '',
    start_date: settings?.evaluation?.start_date || '',
    end_date: settings?.evaluation?.end_date || '',
    observation: settings?.evaluation?.observation || ''
  }
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      try {
        await axios.put(`${settings?.url}`, dataToSend, {
          headers: {
