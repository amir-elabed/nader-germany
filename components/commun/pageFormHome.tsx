...
const Pageform = ({ settings, espace }: any) => {
  ...
  const handleImageFileChange = (event: any) => {
    ...
    if (selectedFile) {
      // Read the selected file and set it as the image preview source
      ...
      // Set the media field in formik
      formik.setFieldValue('media', selectedFile)
    }
  }
  const initialFormValues: any = {
    title: settings?.page?.homePageContent?.title || '',
    media: settings?.page?.homePageContent?.media || '',
    seo_title: settings?.page?.homePageContent?.seo_title || '',
    seo_alias: settings?.page?.homePageContent?.seo_alias || '',
    seo_description: settings?.page?.homePageContent?.seo_description || ''
  }
  ...
  const [title, setTitle] = useState({
    fr: settings?.page?.homePageContent?.fr?.title || '',
    en: settings?.page?.homePageContent?.en?.title || '',
    it: settings?.page?.homePageContent?.it?.title || ''
  })
  ...
  const [languagesliderContent, setLanguagesliderContent] = React.useState({ fr: true, en: false, it: false })
  const [sliderContent, setsliderContent] = useState({
    fr: settings?.page?.homePageContent?.fr?.sliderContent || '',
    en: settings?.page?.homePageContent?.en?.sliderContent || '',
    it: settings?.page?.homePageContent?.it?.sliderContent || ''
  })

  const handleChangesliderContent = (e: any) => {
    if (languagesliderContent?.fr && e !== '<p><br></p>') {
      setsliderContent({ ...sliderContent, fr: e })
    } else if (languagesliderContent?.en && e !== '<p><br></p>') {
      setsliderContent({ ...sliderContent, en: e })
    } else if (languagesliderContent?.it && e !== '<p><br></p>') {
      setsliderContent({ ...sliderContent, it: e })
    }
  }

  const [languageDescription, setLanguageDescription] = React.useState({ fr: true, en: false, it: false })
  const [description, setDescription] = useState({
    fr: settings?.page?.homePageContent?.fr?.description || '',
    en: settings?.page?.homePageContent?.en?.description || '',
    it: settings?.page?.homePageContent?.it?.description || ''
  })
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      if (sliderContent?.fr) formData.append(`fr.sliderContent`, sliderContent?.fr)
      if (sliderContent?.en) formData.append(`en.sliderContent`, sliderContent?.en)
      if (sliderContent?.it) formData.append(`it.sliderContent`, sliderContent?.it)
            ...
            <Grid spacing={2} container>
                ...
                <label htmlFor='media'>
                  <Button
                    ...
                    sx={{
                      ...
                      borderColor: formik?.touched?.media && formik?.errors?.media ? '#d32f2f' : '#E5E5E5',
                  ...
                  >
                    ...
                    {formik?.values?.media && <p>{formik?.values?.media?.name}</p>}
                  </Button>
                </label>
                {/* Display the new image only if it's selected */}
                {imagePreview && <img src={imagePreview} alt='User' width={200} height={100} />}
                {/* Display the saved image if it exists */}
                {settings?.page?.homePageContent?.media &&
                  settings?.page?.homePageContent?.media !== 'undefined' &&
                  !imagePreview && (
                    <img
                      src={`${process.env.NEXT_PUBLIC_API_WITHOUTSLASH}` + settings?.page?.homePageContent?.media}
                      alt='User'
                      width={200}
                      height={100}
                    />
              ...
              <Grid item xs={12} alignItems='center'>
                ...
                <ReactQuill
                  ...
                  value={
                    languagesliderContent?.fr
                      ? sliderContent?.fr
                      : languagesliderContent?.en
                        ? sliderContent?.en
                        : languagesliderContent?.it
                          ? sliderContent?.it
                          : ''
                  }
                  onChange={e => handleChangesliderContent(e)}
                  modules={{
                    toolbar: [
                      [{ header: [1, 2, false] }],
                      ['bold', 'italic', 'underline', 'strike', 'blockquote'],
                ...
                <Box className='languageSelector__wrapper'>
                  <Box className='languageSelector'>
                    <input
                      type='radio'
                      name='languageSelectorsliderContent'
                      id='languageSelectorsliderContent__fr'
                      checked={languagesliderContent?.fr}
                      className='languageSelector__input'
                      onChange={() => setLanguagesliderContent({ fr: true, en: false, it: false })}
                    />
                    <label htmlFor='languageSelectorsliderContent__fr' className='languageSelector__label'>
                      Fr
                    </label>
                  </Box>
                  <Box className='languageSelector'>
                    <input
                      type='radio'
                      name='languageSelectorsliderContent'
                      id='languageSelectorsliderContent__en'
                      className='languageSelector__input'
                      checked={languagesliderContent?.en}
                      onChange={() => setLanguagesliderContent({ fr: false, en: true, it: false })}
                    />
                    <label htmlFor='languageSelectorsliderContent__en' className='languageSelector__label'>
                      En
                    </label>
                  </Box>
                  <Box className='languageSelector'>
                    <input
                      type='radio'
                      name='languageSelectorsliderContent'
                      id='languageSelectorsliderContent__it'
                      className='languageSelector__input'
                      checked={languagesliderContent?.it}
                      onChange={() => setLanguagesliderContent({ fr: false, en: false, it: true })}
                    />
                    <label htmlFor='languageSelectorsliderContent__it' className='languageSelector__label'>
                      It
                    </label>
                  </Box>
                </Box>
